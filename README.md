...
## 💻 주요 개발 내용
### 프론트엔드
- **무한 스크롤**: 팔로우 리스트 조회시 커서 기반 무한 스크롤 구현
- **상태 관리**: React Query를 활용한 서버 상태 관리
- **로딩/에러 관리**: 로딩시, 데이터 없을 시, 예측 불가능한 에러시, 예측 가능한 에러시 등 경우에 따라 각기 다른 UI 컴포넌트 개발

### 백엔드
- **이미지 처리**: 리뷰 이미지, 프로필 이미지 CRUD 및 압축 저장 구현
- **데이터 접근**: JPA와 QueryDSL을 활용한 효율적인 데이터 접근 계층 구현
- **무한 스크롤**: 리뷰 조회시 커서 기반 무한 스크롤 구현
